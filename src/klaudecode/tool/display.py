...
class ToolDisplayManager:
    ...
    def generate_status_text(tool_calls: List[ToolCall]) -> Text:
        ...
        if len(running_tool_calls) == 1:
            return Text.assemble('Running ', (ToolCall.get_display_tool_name(running_tool_calls[0].tool_name), 'bold'), ' ', style=ColorStyle.CLAUDE)
        else:
            tool_counts = {}
    ...
    async def live_display(tool_instances: List['ToolInstance'], tool_calls: List[ToolCall], interrupt_handler: 'InterruptHandler'):
        try:
            status = render_dot_status(status=ToolDisplayManager.generate_status_text(tool_calls))
