...
- `general_purpose.py`, `finder.py`, `review.py` (code-reviewer), `maintenance_review.py` (code-maintenance-reviewer) - Individual sub-agent type definitions.
...
Runtime behavior (in `agent/runtime/sub_agent.py`):
...
sub-agents. Do NOT use the Rewind tool.
...
sub-agents. Do NOT use the Rewind tool.</system-reminder>
```
