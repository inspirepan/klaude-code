...
def _build_style_rules(palette: Palette) -> list[tuple[str, str]]:
    ...
    return [
        ...
        ("tool.result", f"fg:{palette.grey_green}"),
        # Mirrors rich's ``ThemeKey.THINKING`` so the compact-mode reasoning
        # preview below the prompt reads like thinking, not tool output.
        ("thinking", f"italic fg:{palette.grey2}"),
