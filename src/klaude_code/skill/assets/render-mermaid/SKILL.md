...
5. On success, read the image path from the script output and return it to the user using markdown image syntax (e.g., `![Mermaid Architecture Diagram](/tmp/render-mermaid-arch.png)`) so the system can render the image for the user; on failure, relay the error message as-is, correct the Mermaid diagram, and retry.
