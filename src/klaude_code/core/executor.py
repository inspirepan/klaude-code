...
class AgentRuntime:
    ...
    async def _run_bash_task(self, *, session: Session, command: str, task_id: str, session_id: str) -> None:
        ...
        await self._emit_event(events.BashCommandStartEvent(session_id=session_id, command=command))
        ...
        async def _terminate_process(p: asyncio.subprocess.Process) -> None:
            ...
            with contextlib.suppress(Exception):
                await asyncio.wait_for(p.wait(), timeout=BASH_TERMINATE_TIMEOUT_SEC)
                return
        ...
        async def _emit_clean_chunk(chunk: str, *, out_file: TextIO) -> None:
            if not chunk:
                return

            cleaned = _ANSI_ESCAPE_RE.sub("", chunk)
            if cleaned:
                await self._emit_event(events.BashCommandOutputDeltaEvent(session_id=session_id, content=cleaned))
                with contextlib.suppress(Exception):
        ...
        try:
            ...
            with log_path.open("w", encoding="utf-8", errors="replace") as out_file:
                while True:
                    ...
                    await _emit_clean_chunk(to_emit, out_file=out_file)

                if pending:
                    await _emit_clean_chunk(pending, out_file=out_file)
                    pending = ""
            ...
            await self._emit_event(events.BashCommandOutputDeltaEvent(session_id=session_id, content=msg))
        finally:
