...
class _Subscriber:
    subscriber_id: str
    session_id: str | None
    queue: asyncio.Queue[events.Event | _DisconnectSentinel]
...
class EventSubscription:
    ...
    async def _iter_events(self) -> AsyncIterator[events.Event]:
        try:
            while True:
...
class EventBus:
    ...
    def subscribe(self, session_id: str | None) -> EventSubscription:
        ...
        queue: asyncio.Queue[events.Event | _DisconnectSentinel] = asyncio.Queue(maxsize=self._subscriber_queue_maxsize)
