...
| G-002 | 已移除 `Executor`/`ExecutorContext` 兼容层，运行入口统一为 `core/control/app_runtime.py::AppRuntime -> core/control/runtime_hub.py -> SessionRuntime.mailbox` | `RuntimeHub -> SessionRuntime.mailbox` 成为唯一输入调度入口 | 现状 | Phase 2/4 | closed |
