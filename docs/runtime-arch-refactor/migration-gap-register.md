...
| G-003 | `UserInteractionManager` 仍是独立侧通道（已支持多 pending，且已回调同步 session 侧 pending 状态） | interaction 进入 `SessionRuntime.pending_requests` 闭环（无侧通道） | 现状 | Phase 3 | open |
...
| G-010 | SessionRuntime 已有 root-task gate + `RootTaskState(task_id, kind)` + pending request 状态，但仍未形成完整运行态对象 | SessionRuntime 内聚 `active_root_task/pending_requests/config` 完整运行态 | Phase 2 | Phase 2/3 | open |
