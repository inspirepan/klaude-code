...
export function NewSessionButton({ onClick }: NewSessionButtonProps): JSX.Element {
  return (
    <Button
      type="button"
      variant="outline"
      className="h-8 w-full min-w-0 justify-center gap-1.5 overflow-hidden rounded-lg border-neutral-200 px-2.5 text-base font-normal text-neutral-900 hover:bg-muted"
      onClick={() => {
        onClick();
      }}
    >
      <span className="truncate">New</span>
      <span className="inline-flex shrink-0 items-center text-neutral-400" aria-hidden="true">
