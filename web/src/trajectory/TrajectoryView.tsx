...
// klaude: upstream cross-package imports -> the local flattened contract. The
...
// klaude: TrajectoryViewInjected (the upstream plugin inject face) replaced by explicit props

/** History-open lifecycle of a session event window (upstream `OpenState`). */
export type TrajectoryOpenState = 'cold' | 'loading' | 'open' | 'error'

/** The `SessionSnapshot` fields this view reads. */
export interface TrajectorySessionState {
  readonly openState: TrajectoryOpenState
  readonly loadingOlder: boolean
  readonly hasMore: boolean
}

/** One-shot focus request addressed to this view (upstream `ConversationViewRequest`). */
export interface TrajectoryViewRequest {
  /** Target view id; this view answers to `trajectory`. */
  readonly view: string
  /** Target-owned opaque focus identity (a tool call id here). */
  readonly focus: string
}

/** Everything the host supplies to the trajectory view. */
export interface TrajectoryViewProps {
  /** Assembled ledger material for the bound session. */
  snapshot: TrajectorySnapshot
  /** Session lifecycle facts driving the loading and load-earlier affordances. */
  session: TrajectorySessionState
  /** Timeline duration preference (host-persisted). */
  actualDuration: boolean
  /** Record a new duration preference. */
  setActualDuration: (actualDuration: boolean) => void
  /** Fetch one older history page; resolves true when more may still exist. */
  loadOlder: () => Promise<boolean>
  /** Render one group of record images; render nothing to omit them. */
  renderImages: RenderMessageImages
  /** Namespace-bound translator. */
  t: TrajectoryTranslate
  /** Focus request addressed to this view. */
...
// klaude: injected hooks (useSession/useTrajectory/useDuration/renderSlot/loadImage)
// replaced by the props above; the renderImages slot bridge is now a plain prop
export function TrajectoryView({
  ...
  const [actualTime, setActualTime] = useState(false) // klaude: actualDuration is a prop
  ...
  const inspection = snapshot // klaude: was useTrajectory(snapshot => snapshot)
  const historyLoading = session.openState === 'loading' // klaude: was useSession(...)
  const olderHistoryLoading = session.loadingOlder // klaude: was useSession(...)
  const hasOlderHistory = session.hasMore // klaude: was useSession(...)
