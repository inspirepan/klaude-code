...
class TestGlobBase:
    ...
    def setup_fd_mock(self, has_fd):
        ...
        with patch(
            "klaudecode.utils.file_utils.file_glob.FileGlob._has_fd",
            return_value=has_fd,
        ):
            yield
    ...
    def test_glob_result_truncation(self):
        ...
        for line in lines:
            if (
                line.strip()
                and not line.startswith("(Results are truncated")
                and not line.startswith("(Too many files")
                and "Consider:" not in line
            ):
