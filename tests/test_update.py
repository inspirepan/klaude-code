...
def _reset_auto_upgrade_state(monkeypatch) -> None:
...
def test_get_update_message_index_triggers_background_upgrade(monkeypatch) -> None:
    ...
    def _fake_start_auto_upgrade(start_info: update.VersionInfo) -> None:
        ...
        update._auto_upgrade_in_progress = True
        update._auto_upgrade_target_version = "1.1.0"
...
def test_get_update_message_shows_background_upgrade_completed(monkeypatch) -> None:
    _reset_auto_upgrade_state(monkeypatch)
    monkeypatch.setattr(update, "_auto_upgrade_succeeded", True)
    monkeypatch.setattr(update, "_auto_upgrade_target_version", "1.1.0")
...
def test_get_update_message_shows_auto_upgrade_failure(monkeypatch) -> None:
    _reset_auto_upgrade_state(monkeypatch)
    monkeypatch.setattr(update, "_auto_upgrade_failed", True)
    monkeypatch.setattr(update, "_auto_upgrade_attempted", True)
    ...
    monkeypatch.setattr(update, "check_for_updates", lambda: info)
    monkeypatch.setattr(update, "_start_auto_upgrade_if_needed", lambda _info: None)

    message = update.get_update_message()
    assert message == "New version available: 1.1.0. Auto-update failed; run `klaude upgrade`."


def test_get_update_message_editable_skips_background_upgrade(monkeypatch) -> None:
    _reset_auto_upgrade_state(monkeypatch)
