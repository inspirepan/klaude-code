...
class TestBashInteractionDetector(BaseToolTest):
    ...
    def test_interactive_patterns_constant(self):
        ...
        expected_patterns = ['password', 'enter passphrase', 'are you sure', '(y/n)', 'continue?', 'do you want to', 'confirm', "type 'yes'", 'press h for help', 'press q to quit']

        for pattern in expected_patterns:
            assert pattern in BashInteractionDetector.INTERACTIVE_PATTERNS, f"Pattern '{pattern}' missing from INTERACTIVE_PATTERNS"
    ...
    def test_safe_continue_patterns_constant(self):
        ...
        for pattern in expected_patterns:
            assert pattern in BashInteractionDetector.SAFE_CONTINUE_PATTERNS, f"Pattern '{pattern}' missing from SAFE_CONTINUE_PATTERNS"
    ...
    def test_pattern_overlap_handling(self):
        ...
        ambiguous_texts = ['Press enter to continue or type password:', 'Continue? Press enter or (y/n)', 'Enter password or press enter to continue']
    ...
    def test_special_characters_in_prompts(self):
        ...
        for prompt in special_prompts:
            if any(pattern in prompt.lower() for pattern in ['password', 'passphrase', 'are you sure', '(y/n)', "type 'yes'"]):
                result = BashInteractionDetector.detect_interactive_prompt(prompt)
                assert result is True, f"'{prompt}' should be detected as interactive"
