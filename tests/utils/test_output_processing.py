...
class TestBashOutputProcessor(BaseToolTest):
    ...
    def test_process_output_line_interactive_prompt(self):
        ...
        with patch(
            "klaudecode.utils.bash_utils.output_processing.BashInteractionDetector.detect_interactive_prompt"
        ) as mock_detect:
            mock_detect.return_value = True

            new_size, should_break = BashOutputProcessor.process_output_line(
                "Enter password:", output_lines, total_size, update_func
            )
    ...
    def test_process_output_line_safe_continue_prompt(self):
        ...
        with patch(
            "klaudecode.utils.bash_utils.output_processing.BashInteractionDetector.detect_interactive_prompt"
        ) as mock_detect_interactive:
            with patch(
                "klaudecode.utils.bash_utils.output_processing.BashInteractionDetector.detect_safe_continue_prompt"
            ) as mock_detect_safe:
                mock_detect_interactive.return_value = False
                mock_detect_safe.return_value = True

                new_size, should_break = BashOutputProcessor.process_output_line(
                    "Press enter to continue", output_lines, total_size, update_func
                )
    ...
    def test_read_process_output_unix_with_data(self, mock_select):
        ...
        new_size, should_break, error = BashOutputProcessor.read_process_output(
            mock_process, output_lines, total_size, update_func
        )
    ...
    def test_read_process_output_unix_no_data(self, mock_sleep, mock_select):
        ...
        new_size, should_break, error = BashOutputProcessor.read_process_output(
            mock_process, output_lines, total_size, update_func, check_canceled
        )
