...
class TestBashCommandExecutor(BaseToolTest):
    ...
    def test_successful_command_execution(self, mock_env, mock_preprocess, mock_popen):
        ...
        with patch('klaudecode.utils.bash_utils.command_execution.BashOutputProcessor.read_process_output') as mock_read:
            mock_read.return_value = (100, True, '')  # total_size, should_break, error

            check_canceled = Mock(return_value=False)
            update_content = Mock()

            result = BashCommandExecutor.execute_bash_command(command='echo test', timeout_seconds=30.0, check_canceled=check_canceled, update_content=update_content)
        ...
        with patch('klaudecode.utils.bash_utils.command_execution.time.time') as mock_time:
            ...
            with patch('klaudecode.utils.bash_utils.command_execution.BashProcessManager.kill_process_tree') as mock_kill:
                with patch('klaudecode.utils.bash_utils.command_execution.BashOutputProcessor.read_process_output') as mock_read:
                    ...
                    result = BashCommandExecutor.execute_bash_command(command='sleep 10', timeout_seconds=30.0, check_canceled=check_canceled, update_content=update_content)
    ...
    def test_command_cancellation(self, mock_popen):
        ...
        with patch('klaudecode.utils.bash_utils.command_execution.BashProcessManager.kill_process_tree') as mock_kill:
            with patch('klaudecode.utils.bash_utils.command_execution.BashOutputProcessor.read_process_output') as mock_read:
                ...
                result = BashCommandExecutor.execute_bash_command(command='sleep 60', timeout_seconds=120.0, check_canceled=check_canceled, update_content=update_content)
    ...
    def test_command_with_non_zero_exit_code(self, mock_popen):
        ...
        mock_process = Mock()
        mock_process.poll.side_effect = [None, 1, 1]  # Running, then finished with exit code 1
        ...
        with patch('klaudecode.utils.bash_utils.command_execution.BashOutputProcessor.read_process_output') as mock_read:
            ...
            result = BashCommandExecutor.execute_bash_command(command='false', timeout_seconds=30.0, check_canceled=check_canceled, update_content=update_content)
    ...
    def test_process_exception_handling(self, mock_popen):
        ...
        result = BashCommandExecutor.execute_bash_command(command='invalid_command', timeout_seconds=30.0, check_canceled=check_canceled, update_content=update_content)
    ...
    def test_environment_setup(self, mock_env, mock_popen):
        ...
        with patch('klaudecode.utils.bash_utils.command_execution.BashOutputProcessor.read_process_output') as mock_read:
            ...
            BashCommandExecutor.execute_bash_command(command='echo test', timeout_seconds=30.0, check_canceled=check_canceled, update_content=update_content)
    ...
    def test_update_content_callback(self):
        ...
        with patch('klaudecode.utils.bash_utils.command_execution.subprocess.Popen') as mock_popen:
            ...
            with patch('klaudecode.utils.bash_utils.command_execution.BashOutputProcessor.read_process_output') as mock_read:
                ...
                with patch('klaudecode.utils.bash_utils.command_execution.BashOutputProcessor.format_output_with_truncation') as mock_format:
