...
class TestBashEnvironment(BaseToolTest):
    ...
    def test_detect_interactive_prompt(self):
        ...
        for prompt in interactive_prompts:
            assert BashInteractionDetector.detect_interactive_prompt(prompt), (
                f"'{prompt}' should be detected as interactive"
            )
        ...
        for text in non_interactive:
            assert not BashInteractionDetector.detect_interactive_prompt(text), (
                f"'{text}' should NOT be detected as interactive"
            )
    ...
    def test_preprocess_command_without_timeout(self):
        ...
        with unittest.mock.patch(
            "klaudecode.utils.bash_utils.environment.BashEnvironment._has_timeout_command",
            return_value=False,
        ):
            # Simple command
            result = BashEnvironment.preprocess_command("echo test", 5.0)
