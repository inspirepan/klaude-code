...
class TestDebugCommand:
    ...
    async def test_export_openai_schema(self):
        ...
        with pytest.MonkeyPatch().context() as m:
            m.setattr('klaudecode.user_command.command_debug.user_select', AsyncMock(return_value=1))
    ...
    async def test_generate_curl_openai(self):
        ...
        with pytest.MonkeyPatch().context() as m:
            m.setattr('klaudecode.user_command.command_debug.user_select', AsyncMock(return_value=0))
            m.setattr('klaudecode.user_command.command_debug.subprocess.run', Mock())
    ...
    async def test_user_cancel(self):
        ...
        # Mock subprocess.run to prevent any subprocess calls
        with pytest.MonkeyPatch().context() as m:
            m.setattr('klaudecode.user_command.command_debug.user_select', AsyncMock(return_value=None))
            m.setattr('klaudecode.user_command.command_debug.subprocess.run', Mock())
    ...
    def test_render_user_msg_suffix_curl(self):
        ...
        user_msg.set_extra_data(
            'debug_exported',
            {'type': 'curl', 'provider': 'openai', 'file_path': '/test/path.sh', 'message_count': 5, 'tool_count': 3, 'role_counts': {'user': 2, 'assistant': 2, 'tool': 1}},
        )
    ...
    def test_render_user_msg_suffix_schema(self):
        ...
        user_msg.set_extra_data(
            'debug_exported',
            {'type': 'schema', 'provider': 'anthropic', 'file_path': '/test/path.json', 'message_count': 5, 'tool_count': 3, 'role_counts': {'user': 2, 'assistant': 2, 'tool': 1}},
        )
    ...
    def test_build_curl_commands(self):
        ...
        # Test OpenAI curl
        openai_curl = self.debug_command._build_openai_curl('https://api.openai.com/v1/', 'test-key', 'gpt-4', messages, tools, self.mock_config)
        ...
        # Test Anthropic curl
        anthropic_curl = self.debug_command._build_anthropic_curl('https://api.anthropic.com/v1/', 'test-key', 'claude-3', messages, tools, self.mock_config)
