...
def mock_agent_state_with_task():
    ...
    task_call = ToolCall(
        ...
        tool_args_dict={
            ...
            "prompt": "Please analyze the project code structure",
        },
        status="success",
    )
    ...
    tool_msg = ToolMessage(
        ...
        tool_call_cache=task_call,
        content="Code analysis result: Project contains 3 main modules...",
    )
...
def mock_agent_state_with_multiple_conversations():
    ...
    tool_msg2 = ToolMessage(
        tool_call_id="task_456",
        tool_call_cache=task_call2,
        content="Performance analysis result: Found memory leak...",
    )
...
class TestOutputCommand:
    ...
    @patch("os.system")
    @patch("os.makedirs")
    async def test_handle_with_ai_message(
        self, mock_makedirs, mock_os_system, output_command, mock_agent_state, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    @patch("os.system")
    async def test_handle_with_multiple_conversations(
        self,
        mock_os_system,
        output_command,
        mock_agent_state_with_multiple_conversations,
        tmp_path,
    ):
