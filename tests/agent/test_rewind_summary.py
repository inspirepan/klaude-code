...
# ---------------------------------------------------------------------------
# model-Rewind boundary pivots
# ---------------------------------------------------------------------------


def test_build_rewind_instruction_rewind_boundary_quote() -> None:
    history = _history()
    history.insert(
        3,
        message.RewindEntry(
            checkpoint_id=7,
            note="keep A-C",
            rationale="explore branches",
            reverted_from_index=9,
            original_user_message="C",
        ),
    )

    instruction = build_rewind_instruction(history, 3)
    text = message.join_text_parts(instruction.parts)

    assert "<pivot-rewind-boundary>" in text
    assert "Rewind note: keep A-C" in text
    assert "Rationale: explore branches" in text
    assert "<pivot-user-message>" not in text
    # The kept prefix (A) must not leak into the boundary quote.
    assert ">A<" not in text


def test_rewind_op_at_rewind_boundary_keeps_prefix_and_validates_checkpoint(
    tmp_path: Path, isolated_home: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    del isolated_home
    ...
    async def _test() -> None:
        ...
        history = [
            ...
            _user("D"),
            message.AssistantMessage(parts=message.text_parts_from_str("redo work")),
        ]
        ...
        # pivot sits on the RewindEntry itself; anchor = checkpoint id.
        operation = op.RewindWithSummaryOperation(session_id=session.id, pivot_index=3, pivot_checkpoint_id=7)
        await handler.rewind_with_summary(operation)
        await _drain_tasks(spawned, handler)
        ...
        loaded = Session.load(ready[0].new_session_id, work_dir=session.work_dir)
        # Kept prefix: A, assistant reply, C — the RewindEntry itself is
        # summarized away, not carried over.
        assert [type(item).__name__ for item in loaded.conversation_history[:-1]] == [
            "UserMessage",
            "AssistantMessage",
            "UserMessage",
        ]
        assert isinstance(loaded.conversation_history[-1], message.ForkSummaryEntry)
        assert loaded.conversation_history[-1].summary == "POST-REWIND SUMMARY"

    asyncio.run(_test())


def test_rewind_op_rejects_checkpoint_mismatch(
    tmp_path: Path, isolated_home: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    del isolated_home
    ...
    async def _test() -> None:
        ...
        session.append_history(
            [
                ...
                message.RewindEntry(
                    ...
                    note="n",
                    rationale="r",
                    reverted_from_index=2,
                    original_user_message="A",
                ),
            ]
        )
        await session.wait_for_flush()
        ...
        # Checkpoint id does not match the RewindEntry at that index.
        operation = op.RewindWithSummaryOperation(session_id=session.id, pivot_index=1, pivot_checkpoint_id=99)
        await handler.rewind_with_summary(operation)
        await _drain_tasks(spawned, handler)
