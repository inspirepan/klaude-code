"""`klaude web`: read the server's web port and hand the URL to a browser."""
...
def test_web_opens_the_browser_at_the_reported_url(monkeypatch: pytest.MonkeyPatch, opened_urls: list[str]) -> None:
...
def test_web_falls_back_to_the_port_when_url_is_absent(monkeypatch: pytest.MonkeyPatch, opened_urls: list[str]) -> None:
...
def test_web_no_open_only_prints(monkeypatch: pytest.MonkeyPatch, opened_urls: list[str]) -> None:
...
def test_web_print_url_emits_only_the_url(monkeypatch: pytest.MonkeyPatch, opened_urls: list[str]) -> None:
    _stub_status(monkeypatch, {"web_port": 8765, "web_url": "http://127.0.0.1:8765"})

    result = CliRunner().invoke(app, ["web", "--print-url"])
...
def test_web_reports_a_server_without_a_web_port(monkeypatch: pytest.MonkeyPatch, opened_urls: list[str]) -> None:
...
def test_web_reports_an_unreachable_server(monkeypatch: pytest.MonkeyPatch, opened_urls: list[str]) -> None:
...
def test_web_is_listed_in_the_top_level_help() -> None:
