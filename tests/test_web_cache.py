...
from klaude_code.tool.web.web_cache import (
...
class TestCacheGetSet:
    ...
    def test_expired(self) -> None:
        ...
        with patch("klaude_code.tool.web.web_cache.monotonic", return_value=100.0):
            set_cached("key2", "value2")
        # Read at t=1001, expired
        with patch("klaude_code.tool.web.web_cache.monotonic", return_value=1001.0):
            assert get_cached("key2") is None

    def test_not_expired(self) -> None:
        with patch("klaude_code.tool.web.web_cache.monotonic", return_value=100.0):
            set_cached("key3", "value3")
        with patch("klaude_code.tool.web.web_cache.monotonic", return_value=999.0):
            assert get_cached("key3") == "value3"

    def test_max_entries_eviction(self) -> None:
        _cache.clear()
        with patch("klaude_code.tool.web.web_cache.WEB_CACHE_MAX_ENTRIES", 3):
            set_cached("a", 1)
            set_cached("b", 2)
            set_cached("c", 3)
            assert len(_cache) == 3
