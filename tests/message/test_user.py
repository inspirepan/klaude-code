...
from klaudecode.message.user import (
    INTERRUPTED_MSG,
    SpecialUserMessageTypeEnum,
    UserMessage,
    compact_renderer,
    initialize_default_renderers,
    interrupted_renderer,
)
...
class TestUserMessage:
    ...
    def test_get_content_with_pre_system_reminders(self):
        msg = UserMessage(
            ...
            pre_system_reminders=["Pre reminder 1", "Pre reminder 2"],
        )
    ...
    def test_get_content_with_post_system_reminders(self):
        msg = UserMessage(
            content="Main content",
            post_system_reminders=["Post reminder 1", "Post reminder 2"],
        )
    ...
    @patch(
        "klaudecode.message.registry._USER_MSG_CONTENT_FUNCS",
        {"custom_type": lambda msg: f"Custom: {msg.content}"},
    )
    ...
    def test_get_content_all_features(self):
        msg = UserMessage(
            content="Main content",
            pre_system_reminders=["Pre reminder"],
            post_system_reminders=["Post reminder"],
        )
    ...
    @patch(
        "klaudecode.message.registry._USER_MSG_RENDERERS",
        {"special": lambda msg: ["Custom rendered"]},
    )
    ...
    @patch(
        "klaudecode.message.registry._USER_MSG_SUFFIX_RENDERERS",
        {"special": lambda msg: ["Custom suffix"]},
    )
