...
from klaude_code.server.routes.sessions import stream_sessions
from klaude_code.server.session_live import SessionEventStream
...
def test_session_stream_releases_subscription_when_cancelled() -> None:
    async def _run() -> None:
        ...
        response = await stream_sessions(cast(Request, _Request()), state=cast(ServerAppState, state))
